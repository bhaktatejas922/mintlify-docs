...
- **Lightning Fast**: Transform entire documents at 2000+ tokens per second
- **Real-time Streaming**: Users see content being generated as it happens
- **Document-Wide Scope**: Handle complete document restructuring that's impossible with local AI approaches
- **Large Document Handling**: No timeout issues with long documents
- **Consistent Results**: Apply uniform changes across all content sections simultaneously
- **Perfect Integration**: Seamlessly integrates with Plate.js's existing markdown serialization
